...
#if defined(NDEBUG)
# error "Mecoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x8efadb0409c3265339a794b7677258df6ba60221a259d6e2fd33fa362f525e94");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Mecoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Mecoin Signed Message:\n";
...
{
    // Mecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Mecoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Mecoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Mecoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Mecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Mecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xe491dfc375adce47452530fa1a43ce7985b944e2853e3579942efd281f087f0e");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "HN Times 14/July/2018 Dung Vu, Mecoin project";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1531567639;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086221467;

        if (fTestNet)
        {
            block.nTime    = 1531567616;
            block.nNonce   = 386456347;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
{
        ...
        assert(block.hashMerkleRoot == uint256("0xf50c320074ff7fd331649af34bd2be5dabed935651f26f17134568aca6211bb0"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Mecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MecoinMiner(CWallet *pwallet)
{
    printf("MecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("mecoin-miner");
    ...
    try { loop {
        ...
        printf("Running MecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MecoinMiner, pwallet));
}
